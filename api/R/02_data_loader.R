...
load_overall_mape <- function(force = FALSE) {
  ...
  # NOTE: certaines versions de overall_mape.xlsx (l'export national complet)
  # n'incluent pas la colonne "received_on_time" (utilisée pour la
  # promptitude). On la crée à NA plutôt que de planter, et on la laisse en
  # NA (pas de na0()) pour que compute_completude_promptitude() puisse
  # distinguer "donnée indisponible" de "réellement zéro".
  if (!"received_on_time" %in% names(df)) df$received_on_time <- NA_real_
  ...
  df <- df %>%
    ...
    mutate(
      ...
      received_on_time  = as.numeric(received_on_time)
    ) %>%
...
#' si USE_GITHUB_SOURCES est activé (voir R/00_config.R).
#' Par défaut (force=TRUE), retélécharge à CHAQUE appel (donc à chaque
#' démarrage du serveur, cf. refresh_all_data()) même si une copie locale
#' existe déjà : c'est voulu, pour que overall_mape/region/district restent
#' toujours synchronisés avec la dernière version poussée sur GitHub plutôt
#' que de rester bloqués sur une copie locale périmée. Les URLs encore au
#' format placeholder ("<owner>...") sont ignorées silencieusement — ces
#' fichiers (ll_mpox, ll_cholera, epidemie) restent donc en local tant
#' qu'ils ne sont pas publiés sur GitHub.
download_github_sources <- function(force = TRUE) {
